...
var JSONApi = {
	...
	/**
	 * Prepare records and populated associations to be consumed
	 *
	 ...
	 * @return {Object} The returned structure can be consumed when passed to res.json()
	 */
	buildResponse: function ( model, records, associations, sideload, associatedRecords ) {
		...
		var modelIdentity = model.globalId;
		var modelPlural = pluralize( modelIdentity );
		var documentIdentifier = _.kebabCase( modelPlural ); //plural ? modelPlural : modelIdentity;
		var json = {};
		var pk = model.primaryKey;
		var assocPks = {};
		var prefix = sails.config.blueprints.restPrefix || sails.config.blueprints.prefix;
		...
		var prepareOneRecord = function ( record ) {
			...
			_.each( associations, function ( assoc ) {
				...
				if ( assoc.type === "collection" ) {
					...
					var via = _.kebabCase(modelIdentity);
					...
					if ( sideload && assoc.include === "record" && record[ assoc.alias ] && record[ assoc.alias ].length > 0 ) {
						...
						json[ assocModelIdentifier ] = json[ assocModelIdentifier ].concat( JSONApi.linkAssociations( assocModel, record[ assoc.alias ] ) );
				...
				if ( assoc.type === "model" && record[ assoc.alias ] ) {
					if ( sideload && assoc.include === "record" ) {
						...
						assocPk = assocModel.primaryKey;
						var linkedRecords = JSONApi.linkAssociations( assocModel, record[ assoc.alias ] );
